...
// DEFAULT_EMAIL_TEMPLATES, getEmailTemplates, fillPlaceholders now imported from
// ./lib/emailTemplates (Batch 4 Step 2 prep) — behavior unchanged.
...
// PATCH /api/returns/:id stays here — depends on createShiprocketPickup() below.
...
// /api/returns/track/:id moved to server/routes/returns.js (Batch 4 Step 2, Group 4).
...
app.get('/api/health', (req, res) => res.json({ ok: true, version: '3.6.0-features', shiprocket: !!SHIPROCKET_EMAIL, email: !!process.env.RESEND_API_KEY, last_email_error: getLastEmailError() || 'none', last_successful_backup: lastSuccessfulBackupAt || 'none yet this boot', webhook_failures_since_boot: getWebhookFailureCount() }));
